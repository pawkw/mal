...
def mal_equate(args):
    if len(args) < 3:
        if args[0].type == args[1].type:
            if not args[0].type in ['list', 'vector', 'hashmap']:
...
builtins = {
    ...
    'prn': MalType.function(lambda args, _: mal_prn(args), builtin=True),
    'pr-str': MalType.function(lambda args, _: mal_pr_str(args), builtin=True),
    'str': MalType.function(lambda args, _: mal_str(args, " "), builtin=True),
    'println': MalType.function(lambda args, _: mal_println(args), builtin=True),
    'list': MalType.function(lambda args, _: MalType.list(args), builtin=True),
    'list?': MalType.function(lambda args, _: MalType.true() if args[0].isType('list') else MalType.false(), builtin=True),
    'empty?': MalType.function(lambda args, _: MalType.true() if len(args[0].data) < 1 else MalType.false(), builtin=True),
    'count': MalType.function(lambda args, _: MalType.integer(len(args[0].data)), builtin=True),
    '<': MalType.function(lambda args, _: mal_compare(args, operator.lt), builtin=True),
    '<=': MalType.function(lambda args, _: mal_compare(args, operator.le), builtin=True),
    '>': MalType.function(lambda args, _: mal_compare(args, operator.gt), builtin=True),
    '>=': MalType.function(lambda args, _: mal_compare(args, operator.ge), builtin=True),
    '=': MalType.function(lambda args, _: mal_equate(args), builtin=True)
}